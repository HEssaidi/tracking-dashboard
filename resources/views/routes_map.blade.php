...
<script type="text/javascript">
  var routes=[];
  var data_array_coords=document.getElementById("div_coords");
  if (data_array_coords.hasChildNodes()) {
     var children = data_array_coords.childNodes;

  for (var i = 0; i < children.length; i++) {
    if (children[i].innerText){
      routes.push({lat: parseFloat(children[i].innerText), lng: parseFloat(children[i+1].innerText) });
      console.log(children[i].innerText);
      console.log(children[i+1].innerText);
    }
  }
}
